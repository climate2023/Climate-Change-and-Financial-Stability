...
ventana.traslape        <- c('0')   #<<<--- Puede ser 50, 100 o 150   (Importante que sea string)
...
load(paste0(directorio.saved,'Nino/',tipo.serie,'_tra',ventana.traslape,'_est',ventana.estimacion,'_',tipo.bei,'_',plazo.bei,'_year.RData'))
...
for(ind in seq_along(lista.resultados.regresion)) {
  ...
  # Esta parte del codigo no se utiliza con BEI porque no se divide por dos columnas.
  if(0){
  ...
  for(i in seq_along(lista.separada)){
    ...
    save(tabla.bmp, tabla.wilcoxon, tabla.bmp.completo, tabla.wilcoxon.completo,
           file = paste0(directorio.saved,'Nino/Tablas/Tablas_',tipo.serie,'_tra',ventana.traslape,'_est', ventana.estimacion, '_',tipo.bei,'_', plazo.bei,'_', 
