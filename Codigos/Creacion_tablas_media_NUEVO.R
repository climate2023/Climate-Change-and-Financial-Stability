...
for(ventana.estimacion in ventanas.estimacion){
  for(ventana.traslape in ventanas.traslape){
    ...
    if(tipo.serie == 'CDS' | tipo.serie == 'Indices') load(paste0(directorio.saved,tipo.serie,'_tra',ventana.traslape,'_est',ventana.estimacion,'_media_',regresor.mercado,'.RData'))
    if(tipo.serie == 'BEI') load(paste0(directorio.saved,tipo.serie,'_tra',ventana.traslape,'est',ventana.estimacion,'_',))
