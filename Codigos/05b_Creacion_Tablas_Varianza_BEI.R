...
if(tipo.serie == 'BEI'){
  ...
  tipos.bei        <- c('sinprima')    #<<<--- parametro que indica cuales BEI se analizan, los 'originales' o 'sinprima'
  plazo.bei        <- 1                             #<<<--- que plazo de BEI se analiza, 1 2 3 4 o 5
  controles        <- 'Desanclaje_PIB'             #<<<--- parametro que indica los controles que se desean agregar, NULL significa que se hara el analisis para todos los casos
                                       # Es decir, sincontroles, con desanclaje y con desanclaje pib. Si se desea que se haga respecto a solamente uno, es necesario 
                                       # escribirlo, ya sea "Sin_regresoras", "Desanclaje" o "Desanclaje_PIB". Tambien sirve para dos de ellos, por ejemplo c("Desanclaje, "Desanclaje_PIB")
}
...
for(tipo.bei in tipos.bei){
  for(ventana.estimacion in ventanas.estimacion){
    for(ventana.traslape in ventanas.traslape){
      ...
      # Saltar el loop cuando sea el caso 500 E 50 T porque ya se tiene
      if(ventana.estimacion == 500 & ventana.traslape == '50') next
      if(ventana.estimacion == 500 & ventana.traslape == '100') next
      ...
      # Reducir <lista.resultados.regresion>, segun el valor de controles
      if(!is.null(controles)) lista.resultados.regresion <- lista.resultados.regresion[controles]
