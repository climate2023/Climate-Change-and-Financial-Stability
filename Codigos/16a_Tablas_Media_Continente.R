##########################################################
# Codigo para generar las tablas de los tests de la media para continente
...
load(paste0(directorio.saved,tipo.serie,'_tra',ventana.traslape,'_est',ventana.estimacion,'_media_',regresor.mercado,'.RData'))
...
all.events.list <- lapply(all.events.list, function(event){
  event@evento$Continent <- est_continent(event@evento$Country)
  if(is.na(event@evento$Continent)) event <- NA # Eliminar eventos que no les corresponda continente (Sudafrica)
  return(event)
})

# Eliminar eventos NA
suppressWarnings(all.events.list <- purrr::discard(all.events.list, is.na))
...
if(columna.agrupar == 'Ambas'){
  # En primer lugar por cada desastre se necesita una combinacion del continente y el tipo de desastre
  ...
  # Ahora si podemos separar <all.events.list> en listas dependiendo del desastre y el pais donde sucedio
  lista.separada <- split(all.events.list, sapply(all.events.list, function(x) x@evento[['Desastre.Continent']]))
  ...
  # a guardar los elementos de <lista.separada> que contengan mas de un desastre
...
save(tabla.bmp, tabla.wilcoxon,tabla.bmp.completo, tabla.wilcoxon.completo,
     file=paste0(directorio.guardar,'nuevos_modelos/',tipo.serie,'_tra',ventana.traslape,'_est',ventana.estimacion,'_media_',regresor.mercado,'_',agrupacion,'.RData'))
